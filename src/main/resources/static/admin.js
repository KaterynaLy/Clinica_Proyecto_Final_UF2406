...
// Listar citas pendientes (Administrador)
async function listarCitasPendientes() {
    ...
    citas.forEach(cita => {
        const li = document.createElement('li');
        li.textContent = `Cita ID: ${cita.id} - ${cita.fecha} ${cita.hora} - Estado: ${cita.estado}`;
        
        const aprobarBtn = document.createElement('button');
        aprobarBtn.textContent = 'Aprobar';
        aprobarBtn.onclick = () => gestionarCita(cita.id, 'Aprobada');
        
        const rechazarBtn = document.createElement('button');
        rechazarBtn.textContent = 'Rechazar';
        rechazarBtn.onclick = () => gestionarCita(cita.id, 'Rechazada');
...
// Gestionar el estado de la cita (Administrador)
async function gestionarCita(id, estado) {
    ...
    if (response.ok) {
        ...
        alert('Error al gestionar la cita');
    }
}
